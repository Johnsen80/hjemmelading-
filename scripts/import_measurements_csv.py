    ...
    with open(path, newline='', encoding='utf-8') as f:
        ...
        for r in reader:
            ...
            rows.append({
                ...
                'neck_thickness_mm': tofloat(get('neck_thickness_mm') or r.get('neck_thickness')),
                'case_weight_gr': tofloat(get('case_weight_gr') or r.get('case_weight')),
                'passed_qc': 1 if (get('passed_qc') or r.get('passed')) in ('1','true','True','yes','y','Y') else None,
                'notes': r.get('notes') or ''
            })
    return rows


def find_or_create_lot(db, lot_id: int = None, lot_number: str = None, component_type: str = 'bullet'):
    if lot_id:
    ...
    if lot_number:
        ...
        rows = db.execute_query("SELECT * FROM inventory_lots WHERE lot_number = ?", (lot_number,))
        if rows:
            return rows[0]['id']
...
def main():
    ...
    p.add_argument('--lot-number', type=str, help='Lot number (will create if not exists)')
    p.add_argument('--component-type', type=str, default='bullet', help='component_type when creating lot')
    p.add_argument('--outlier-method', type=str, default='mad', choices=['z','mad'], help='Outlier detection method')
    p.add_argument('--outlier-thresh', type=float, default=3.5, help='Threshold for outlier detection')
    args = p.parse_args()
    ...
    db = get_database()
    lot_id = find_or_create_lot(db, lot_id=args.lot_id, lot_number=args.lot_number, component_type=args.component_type)
    ...
    session_id = db.create_measurement_session(lot_id, measured_by='csv_import', sample_size=len(rows), measured_all=0, notes=f'Imported from {os.path.basename(args.csv)}')
    print('Created session id', session_id)

    weights = []
    for r in rows:
        db.add_measurement_value(session_id, r['item_index'] or 0, r['weight_grains'], r['length_mm'], r['neck_thickness_mm'], r['case_weight_gr'], r['passed_qc'], r['notes'])
    ...
    print('Stats:', stats)
    if weights:
        outlier_idx = detect_outliers(weights, method=args.outlier_method, thresh=args.outlier_thresh)
