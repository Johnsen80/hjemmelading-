...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
                             ...
                             QLineEdit, QDoubleSpinBox, QGroupBox, QFrame,
...
class ModernLoadBuilder(QWidget):
    ...
    def create_step1_page(self):
        ...
        subtitle.setStyleSheet("color: #7f8c8d; font-size: 12pt; padding-left: 20px; padding-bottom: 20px;")
    ...
    def create_rifle_selection_panel(self):
        ...
        if rifles:
            ...
            no_rifles.setStyleSheet("color: #e67e22; font-style: italic; padding: 20px;")
    ...
    def create_component_controls(self):
        ...
        self.bullet_info.setStyleSheet("color: #7f8c8d; font-size: 9pt; font-style: italic;")
        ...
        self.powder_info.setStyleSheet("color: #7f8c8d; font-size: 9pt; font-style: italic;")
        ...
        self.primer_info.setStyleSheet("color: #7f8c8d; font-size: 9pt; font-style: italic;")
    ...
    def create_visualization_panel(self):
        ...
        self.scope_comparison_group = QGroupBox("🎯 KIKKERT JUSTERING (vs. forrige ladning)")
        ...
        self.scope_comparison_label = QLabel("Velg rifle og komponenter for å se sammenligning...")
        self.scope_comparison_label.setWordWrap(True)
        self.scope_comparison_label.setStyleSheet("color: #856404; font-weight: normal; padding: 5px; font-size: 9pt;")
        ...
        self.scope_comparison_label = QLabel("Velg rifle for å se sammenligning med forrige ladning.")
        self.scope_comparison_label.setWordWrap(True)
        self.scope_comparison_label.setStyleSheet("color: #856404; font-weight: normal; padding: 10px; font-size: 9pt;")
    ...
    def create_stats_bar(self):
        ...
        for label in [self.stat_pressure, self.stat_velocity, self.stat_energy,
                     self.stat_barrel_time, self.stat_safety]:
    ...
    def get_powder_recommendation_text(self):
        ...
        return recommendations.get(caliber, f"For {caliber}, consult load manuals for powder recommendations.")
    ...
    def load_bullets_for_caliber(self):
        ...
        self.bullet_combo.addItem("Berger 175gr OTM (Lot ABC123)", {'id': 1, 'weight': 175})
        self.bullet_combo.addItem("Sierra 168gr HPBT (Lot XYZ456)", {'id': 2, 'weight': 168})
    ...
    def load_primers(self):
        ...
        for primer in primers:
            self.primer_combo.addItem(f"{primer['manufacturer']} {primer['name']}", primer)
    ...
    def on_powder_changed(self, index):
        ...
        if powder:
            ...
            self.powder_info.setText(f"Type: {powder.get('type', '?')}, In stock: {powder.get('quantity_grams', 0):.0f}g")
    ...
    def update_pressure_graph(self, result):
        ...
        self.pressure_plot.addLine(y=max_pressure, pen=mkPen(color='#95a5a6', width=2, style=Qt.PenStyle.DashLine))
    ...
    def update_velocity_graph(self, result):
        ...
        self.velocity_plot.plot(positions, velocities, pen=mkPen(color='#27ae60', width=3))
    ...
    def update_stats(self, result):
        ...
        self.stat_safety.setStyleSheet(f"color: {color}; font-weight: bold; padding: 8px; font-size: 10pt;")
    ...
    def update_scope_comparison(self, current_result):
        ...
        prev_name, prev_vel, prev_bc, prev_weight, prev_cal, prev_created = previous_load[0]
        ...
        for dist in test_distances:
            # Drop for new load
            drop_new = ballistics_calc.calculate_drop(current_velocity, current_bc, dist, zero_dist, "G1")
            drop_new_moa = ballistics_calc.cm_to_moa(drop_new, dist)
            
            # Drop for previous load
            drop_prev = ballistics_calc.calculate_drop(prev_vel, prev_bc, dist, zero_dist, "G1")
...
💡 TIP: Hvis du jakter i varmt OG kaldt vær, velg temp-stabilt krutt!"""
    ...
    def explain_burn_rate(self):
        ...
        return """🔥 BRENNHASTIGHET (Burn Rate):
...
🎯 FOR DIN {caliber}:
""" + (f"• Brennhastighet 110-120 (medium)\n• Varget (115) = perfekt!\n• H4350 (120) = også bra" if hasattr(self, 'rifle_data') and self.rifle_data and '.308' in self.rifle_data.get('caliber', '') else "• Velg våpen først for spesifikke anbefalinger") + """
...
💡 TIP FOR DIN LADNING:
""" + (f"Med {self.powder_data['name']}: Standard tennhette anbefales\n\n" if hasattr(self, 'powder_data') and self.powder_data else "Velg krutt først for spesifikk anbefaling\n\n") + """🥇 MINE FAVORITTER:
