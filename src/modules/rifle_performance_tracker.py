...
class RiflePerformanceTracker(QWidget):
    ...
    def create_cold_bore_tab(self):
        ...
        for label, h, v in [("On Zero", 0, 0), ("1 cm høyre", 1, 0), 
                            ("2 cm opp", 0, 2), ("1↗", 1, 1)]:
            btn = QPushButton(label)
            btn.clicked.connect(lambda checked, x=h, y=v: (
                self.cb_poi_h.setValue(x), self.cb_poi_v.setValue(y)
            ))
        ...
        save_cb_btn.setStyleSheet("font-size: 14pt; font-weight: bold; background-color: #2196F3; color: white;")
        ...
        self.cb_history_table.setHorizontalHeaderLabels([
            "Dato", "Rifle", "Ammo", "Distanse", "Temp", "POI H (cm)", "POI V (cm)", "Notater"
        ])
    ...
    def create_barrel_tracker_tab(self):
        ...
        # Graf placeholder
        self.bt_chart_label = QLabel("<i>Velg en rifle for å se presisjon vs skudd-antall</i>")
    ...
    def load_rifles(self, combo_widget):
        """Laster rifles"""
        rifles = self.db.execute_query("SELECT id, name, caliber FROM rifles ORDER BY name")
    ...
    def load_ammo_profiles(self, combo_widget):
        ...
        ammos = self.db.execute_query("SELECT id, name, caliber FROM ammo_profiles ORDER BY name")
    ...
    def load_barrel_status(self):
        ...
        try:
            ...
            since_cleaning = last_clean[0][0] if last_clean and len(last_clean) > 0 else total_rounds
            ...
            avg = avg_group[0][0] if avg_group and len(avg_group) > 0 and avg_group[0][0] else None
            
            # Update cards
            self.bt_total_rounds.findChild(QLabel, "value").setText(str(total_rounds))
            self.bt_last_cleaning.findChild(QLabel, "value").setText(str(since_cleaning))
            ...
            self.bt_condition.setStyleSheet(f"""
                QGroupBox {{
    ...
    def add_barrel_rounds(self):
        ...
        group_size = self.bt_group_size.value() if self.bt_group_size.value() > 0 else None
        ...
        try:
            ...
            QMessageBox.information(
                self, "Lagret!",
                f"{rounds} skudd lagt til barrel log!"
            )
    ...
    def run_analysis(self):
        ...
        try:
            ...
            rifle_data = self.db.execute_query(
                "SELECT name, caliber FROM rifles WHERE id = ?",
                (rifle_id,)
            )
...
"""
                if abs(avg_h) < 1 and abs(avg_v) < 1:
                    ...
                    cb_text += "<p style='color: orange;'>⚠️ Moderat cold bore shift. Hold {:.1f}cm mot venstre og {:.1f}cm ned for første skudd.</p>".format(-avg_h, -avg_v)
                else:
