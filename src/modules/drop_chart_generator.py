...
class DropChartGenerator(QWidget):
    ...
    def create_drop_chart_tab(self):
        ...
        self.scope_adjustment_group = QGroupBox("🎯 KIKKERT JUSTERING (vs. forrige ladning)")
        ...
        self.scope_adjustment_label = QLabel("Velg ammunisjon for å se sammenligning med forrige ladning for samme rifle.")
    ...
    def on_ammo_selected(self):
        ...
        prev_name, prev_vel, prev_bc, prev_weight, prev_cal, prev_created = previous_ammo[0]
        ...
        for dist in test_distances:
            # Drop for ny ladning
            drop_new = self.ballistics_calc.calculate_drop(velocity, bc, dist, zero_dist, "G1")
            drop_new_moa = self.ballistics_calc.cm_to_moa(drop_new, dist)
            
            # Drop for forrige ladning
            drop_prev = self.ballistics_calc.calculate_drop(prev_vel, prev_bc, dist, zero_dist, "G1")
    ...
    def generate_drop_chart(self):
        ...
        for idx, distance in enumerate(distances):
            # Drop
            drop_cm = self.ballistics_calc.calculate_drop(velocity, bc, distance, zero, "G1")
            ...
            # Velocity
            if self.drop_include_velocity.isChecked():
                vel = self.ballistics_calc.calculate_velocity_at_distance(velocity, bc, distance, "G1")
    ...
    def export_drop_chart(self, format_type):
        ...
        if format_type == "csv":
            ...
            if filename:
                ...
                QMessageBox.information(self, "Lagret!", f"Drop chart lagret til {filename}")
