...
from PyQt6.QtWidgets import QWidget, QVBoxLayout, QLabel, QComboBox, QPushButton, QFileDialog, QTextEdit, QHBoxLayout, QMessageBox
...
class LotComparisonWidget(QWidget):
    ...
    def compare_lots(self):
        ...
        avg1 = sum(y1)/len(y1)
        avg2 = sum(y2)/len(y2)
        self.result_label.setText(f"Snitt treffgruppe Lot 1: {avg1:.1f} mm\nSnitt treffgruppe Lot 2: {avg2:.1f} mm")

    def upload_image_1(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Velg bilde Lot 1", "", "Image Files (*.png *.jpg *.jpeg)")
        if file_path:
            pixmap = QPixmap(file_path)
            self.image_label_1.setPixmap(pixmap.scaled(200, 200))

    def upload_image_2(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Velg bilde Lot 2", "", "Image Files (*.png *.jpg *.jpeg)")
        if file_path:
            pixmap = QPixmap(file_path)
    ...
    def analyze_images(self):
        ...
        self.analysis_result_label.setText("Bildeanalyse: (kommer, AI/algoritme kan måle gruppe, avstand, treffpunkt osv)")
    ...
    def export_report(self):
        ...
        QMessageBox.information(self, "Rapport", "Rapport generert og klar til eksport.")
