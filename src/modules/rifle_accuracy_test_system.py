...
class RifleAccuracyTestManager(QWidget):
    ...
    def init_ui(self):
        ...
        self.btn_new_test.setStyleSheet("background-color: #27ae60; color: white; padding: 8px; font-weight: bold;")
        
        self.btn_view = QPushButton("🔍 Vis Detaljer")
        self.btn_view.clicked.connect(self.view_test_details)
        self.btn_view.setStyleSheet("background-color: #3498db; color: white; padding: 8px;")
        
        self.btn_print_sheet = QPushButton("🖨️ Print Test Ark")
        self.btn_print_sheet.clicked.connect(self.print_test_sheet)
        self.btn_print_sheet.setStyleSheet("background-color: #9b59b6; color: white; padding: 8px;")
        
        self.btn_chart = QPushButton("📈 Utviklingskurve")
        self.btn_chart.clicked.connect(self.show_development_chart)
        self.btn_chart.setStyleSheet("background-color: #e67e22; color: white; padding: 8px;")
        ...
        self.table.setHorizontalHeaderLabels([
            "ID", "Dato", "Skudd ved Test", "Grupper", "Skudd/Gruppe",
            "Avg Gruppe (mm)", "Avg MOA", "ES fps", "SD fps", "Status"
        ])
    ...
    def load_tests(self):
        ...
        for row, test in enumerate(tests):
            ...
            self.table.setItem(row, 2, QTableWidgetItem(str(test.get('round_count_at_test', 0))))
            self.table.setItem(row, 3, QTableWidgetItem(str(test.get('groups_fired', 0))))
            self.table.setItem(row, 4, QTableWidgetItem(str(test.get('shots_per_group', 0))))
            
            avg_size = test.get('average_group_size_mm', 0)
            self.table.setItem(row, 5, QTableWidgetItem(f"{avg_size:.2f}" if avg_size else "-"))
            
            avg_moa = test.get('average_moa', 0)
            self.table.setItem(row, 6, QTableWidgetItem(f"{avg_moa:.3f}" if avg_moa else "-"))
...
class AccuracyTestDialog(QDialog):
    ...
    def __init__(self, parent=None, rifle_id: int = None, test_id: Optional[int] = None):
        super().__init__(parent)
        self.db = get_database()
        self.rifle_id = rifle_id
        self.test_id = test_id
        
        self.setWindowTitle("🎯 Ny Accuracy Test" if test_id is None else "✏️ Rediger Test")
    ...
    def create_load_data_tab(self):
        ...
        for case in cases:
            self.input_case_id.addItem(f"{case['name']} ({case['manufacturer']})", case['id'])
        ...
        for powder in powders:
            self.input_powder_id.addItem(f"{powder['name']} ({powder['manufacturer']})", powder['id'])
    ...
    def create_groups_tab(self):
        ...
        info = QLabel("🎯 <b>Gruppestørrelser</b> - Mål hver gruppe i mm (eller bruk MOA)")
        info.setStyleSheet("font-size: 14px; margin-bottom: 10px;")
        layout.addWidget(info)
    ...
    def calculate_group_stats(self):
        ...
        self.label_best_group.setText(f"{best:.2f} mm ({(best/distance_m)*34.38:.3f} MOA)")
        self.label_worst_group.setText(f"{worst:.2f} mm ({(worst/distance_m)*34.38:.3f} MOA)")
    ...
    def save_test(self):
        ...
        if not group_sizes:
            QMessageBox.warning(self, "Mangler Data", "Legg inn minst én gruppestørrelse.")
            return
        ...
        if velocities:
            ...
            variance = sum((x - avg_velocity) ** 2 for x in velocities) / len(velocities)
            sd = variance ** 0.5
...
class AccuracyTestDetailsDialog(QDialog):
    ...
    def __init__(self, parent=None, test_id: int = None):
        super().__init__(parent)
        self.db = get_database()
        self.test_id = test_id
