...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                            ...
                            QSpinBox, QTabWidget, QWizard, QWizardPage,
...
class HarmonicWizard(QWidget):
    ...
    def init_ui(self):
        ...
        new_test_btn.setStyleSheet("background-color: #5cb85c; color: white; font-weight: bold;")
    ...
    def analyze_test(self):
        ...
        test = self.db.execute_query(
            "SELECT * FROM seating_depth_tests WHERE id = ?",
            (test_id,)
        )[0]
    ...
    def save_test(self, result):
        ...
        if result == QWizard.DialogCode.Accepted:
            ...
            for coal in coal_values:
                self.db.insert("seating_depth_results", {
                    'test_id': test_id,
                    'coal': coal,
                    'group_size_mm': None,
                    'velocity_avg': None,
    ...
    def __init__(self, parent=None):
        ...
        for bullet in bullets:
            self.bullet_combo.addItem(
                f"{bullet['name']} - {bullet['weight_grains']}gr",
                bullet['id']
            )
...
class HarmonicAnalysisDialog(QDialog):
    ...
    def create_analysis_tab(self):
        ...
        if len(coal_values) < 3:
            layout.addWidget(QLabel("⚠️ For lite data for å kjøre analyse. Legg inn flere resultater."))
            return widget
        ...
        if len(coal_values) >= 4:
            ...
            try:
                spl = UnivariateSpline(coal_values, group_sizes, k=3, s=len(coal_values)*2)
        ...
        if valid_sd:
            ...
            ax2.set_title('Hastighets-konsistens vs Settedybde', fontsize=12, fontweight='bold')
        ...
        group_norm = 1 - (group_norm - min(group_norm)) / (max(group_norm) - min(group_norm) + 0.001)
        
        ax3.plot(coal_values, group_norm, 'b-o', linewidth=2, markersize=8, label='Presisjon (normalisert)')
        
        # Finn lokale maksima (= gode områder)
        if len(group_norm) >= 3:
            peaks, properties = find_peaks(group_norm, prominence=0.1)
            if len(peaks) > 0:
                ax3.plot(np.array(coal_values)[peaks], group_norm[peaks], 'r*', 
                        markersize=15, label='Potensielle noder')
        
        ax3.set_xlabel('COAL (mm)', fontsize=11)
        ax3.set_ylabel('Presisjonsscore (høyere = bedre)', fontsize=11)
        ax3.set_title('AI-identifiserte "Harmoniske Noder"', fontsize=12, fontweight='bold')
        ax3.legend()
    ...
    def create_recommendation_tab(self):
        ...
        group_norm = np.array(group_sizes)
        group_norm_inv = 1 - (group_norm - min(group_norm)) / (max(group_norm) - min(group_norm) + 0.001)
        ...
        if len(peaks) > 0:
            ...
            sorted_peaks = sorted(zip(peaks, peak_scores), key=lambda x: x[1], reverse=True)
            ...
            rec_html += "<p>⚠️ Ingen tydelige harmoniske noder funnet. Dette kan bety:</p><ul>"
            rec_html += "<li>For få testpunkter</li>"
            rec_html += "<li>For stort steg mellom COAL-verdier</li>"
            rec_html += "<li>Våpenet er ikke følsomt for settedybde i dette området</li></ul>"
