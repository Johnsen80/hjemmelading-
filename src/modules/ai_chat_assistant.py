...
class AIMessage(QFrame):
    ...
    def __init__(self, text: str, is_user: bool = False, timestamp: str = None):
        super().__init__()
        self.text = text
...
class AIWorker(QThread):
    ...
    def _generate_response(self, prompt: str, context: Dict) -> str:
        ...
        elif 'temperature' in prompt_lower or 'temp' in prompt_lower or 'seasonal' in prompt_lower:
            return """
...
class AIChatAssistant(QWidget):
    ...
    def init_ui(self):
        ...
        btn_sd.clicked.connect(lambda: self.send_quick_prompt("What is Standard Deviation and what's a good SD for precision shooting?"))
        quick_layout.addWidget(btn_sd)
        
        btn_pressure = QPushButton("Pressure signs?")
        btn_pressure.clicked.connect(lambda: self.send_quick_prompt("How do I interpret pressure signs?"))
        quick_layout.addWidget(btn_pressure)
        
        btn_recommend = QPushButton("Best load?")
        btn_recommend.clicked.connect(lambda: self.send_quick_prompt("Recommend the best load based on my historical data"))
