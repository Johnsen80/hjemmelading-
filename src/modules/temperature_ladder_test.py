...
class TemperatureLadderTest(QWidget):
    ...
    def init_ui(self):
        ...
        self.table_data.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
    ...
    def create_new_test(self):
        ...
        QMessageBox.information(self, "Test opprettet", f"Test ID: {self.current_test_id}")
    
    def load_existing_test(self):
        """Last eksisterende test"""
        # TODO: Implement test selection dialog
        QMessageBox.information(self, "Under utvikling", "Load test dialog kommer snart!")
    ...
    def add_data_point(self):
        ...
        self.table_data.setItem(row, 4, QTableWidgetItem(datetime.now().strftime("%Y-%m-%d %H:%M")))
        ...
        QMessageBox.information(self, "Lagt til", f"Datapunkt lagt til: {temp}°C @ {velocity} fps")
    ...
    def analyze_data(self):
        ...
        ss_res = sum((y - (slope * x + intercept))**2 for x, y in zip(temps, velocities))
        ...
        ax.plot(temp_range, vel_pred, 'r--', linewidth=2, label=f'Trend: {slope:.2f} fps/°C')
        ...
        <ul>
            ...
            <li><b>Rating:</b> <span style='color: {temp_stability_rating[1]};'><b>{temp_stability_rating[0]}</b></span></li>
        </ul>
    ...
    def export_results(self):
        ...
        if filename:
            with open(filename, 'w') as f:
                f.write(f"Temperature Ladder Test Results\n")
                f.write(f"================================\n\n")
                ...
                for d in self.test_data:
                    f.write(f"{d['temp']:.1f}°C: {d['velocity']} fps (ES: {d['es']}, SD: {d['sd']})\n")
            
            QMessageBox.information(self, "Eksportert", f"Resultater lagret til:\n{filename}")
