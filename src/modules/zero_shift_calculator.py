...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
                            ...
                            QTableWidgetItem, QDoubleSpinBox, QTextEdit, QSpinBox)
...
<b>Anbefaling:</b>
  ...
  Test på kort hold først for å bekrefte.
        ...
        for i, row in enumerate(table_data):
            self.distance_table.setItem(i, 0, 
                QTableWidgetItem(f"{row['distance_m']} m"))
            self.distance_table.setItem(i, 1, 
                QTableWidgetItem(f"{row['difference_cm']:.1f} cm"))
            self.distance_table.setItem(i, 2, 
                QTableWidgetItem(f"{row['adjustment_value']:.2f} {click_unit}"))
            
            clicks_str = f"{abs(row['clicks'])} {row['direction']}"
            self.distance_table.setItem(i, 3, 
                QTableWidgetItem(clicks_str))
