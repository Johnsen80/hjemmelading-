...
class WorkflowCard(QFrame):
    ...
    def __init__(self, workflow_id: str, title: str, description: str,
                 icon: str, color: str, status: str = "ready", parent=None):
...
class WorkflowHub(QWidget):
    ...
    def init_ui(self):
        ...
        title.setStyleSheet("font-size: 32px; font-weight: bold; color: #e0e0e0; margin-bottom: 0px; letter-spacing: 2px;")
        title.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        hero_text.addWidget(title)
        subtitle = QLabel("Premium reloading, ballistics & analysis. Velg modul for å komme i gang.")
        subtitle.setStyleSheet("font-size: 17px; color: #ffd700; margin-bottom: 10px; font-weight: 600;")
        ...
        self.mode_description.setStyleSheet("color: #7f8c8d; font-size: 11px; font-style: italic;")
        ...
        grid.setSpacing(24)
        premium_mods = [
            ("dashboard", "Dashboard", "Oversikt, AI-tips, widgets og statistikk", "#ffd700", "dashboard_icon.png"),
            ("batch_qc", "Batch QC", "Batch-logging, analyse og rapportering", "#27ae60", "batch_icon.png"),
            ("target_analyzer", "Bildeanalyse", "Automatisk skivegjenkjenning og treffanalyse", "#00bfff", "target_icon.png"),
            ("rifle_optic_manager", "Våpen & Optikk", "Profil, ballistikk og utstyr", "#bfa14a", "rifle_icon.png"),
        ]
        for i, (wf_id, title, desc, color, icon_file) in enumerate(premium_mods):
            card = QWidget()
            card_layout = QVBoxLayout()
            card.setLayout(card_layout)
            card.setStyleSheet(f"background-color: #23242b; border-radius: 18px; border: 2px solid {color}; padding: 24px;")
            # Ikon
            ...
            # Tittel
            title_label = QLabel(title)
            title_label.setStyleSheet(f"font-size: 22px; font-weight: bold; color: {color}; margin-top: 8px;")
            title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            card_layout.addWidget(title_label)
            # Beskrivelse
            desc_label = QLabel(desc)
            desc_label.setStyleSheet("font-size: 13px; color: #e0e0e0; margin-bottom: 8px;")
            desc_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            card_layout.addWidget(desc_label)
            # Start-knapp
            btn = QPushButton("Start")
            btn.setStyleSheet(f"background-color: {color}; color: #23242b; font-weight: bold; border-radius: 8px; padding: 10px 24px; font-size: 15px;")
            btn.setCursor(Qt.CursorShape.PointingHandCursor)
            btn.clicked.connect(lambda _, wf_id=wf_id: self.workflow_selected.emit(wf_id))
        ...
        workflows = [
            ("load_development_workflow", "🎯", "Load Development Workflow", "KOMPLETT: Load → Batch → Test → Analyze → Optimize → Finalize"),
        ]
        ...
        testing = [
            ("ocw_test", "▲", "OCW Test", "Optimal Charge Weight - find pressure nodes"),
            ("ladder_test", "▲▲", "Ladder Test", "Velocity ladder analysis"),
            ("seating_depth", "◆", "Seating Depth Test", "Berger method - find best CBTO/jump"),
        ]
        ...
        components = [
            ("component_database", "■", "Component Database", "Bullets, powder, primers, brass inventory"),
            ("component_inventory", "▣", "Component Inventory", "Stock levels, costs, lot tracking"),
            ("primer_tools", "◆", "Primer Tools", "Selector, seating guide, pressure diagnostics"),
        ]
        ...
        components = [
            ("brass_manager", "🥉", "Brass/Hylse Manager", "Lifecycle tracking: Kjøp → Firing → Annealing → Retirement"),
            ("bullet_manager", "🎯", "Bullet Manager", "Spor bullet lots, BC testing, sorting"),
            ("powder_manager", "💨", "Powder Manager", "Lot tracking, temp sensitivity, burn rate"),
        ...
        equipment = [
            ("rifle_optic_manager", "🎯", "Rifles & Optikk", "Administrer våpen og kikkertsikter"),
            ("ammo_profile_manager", "📦", "Ammunisjonsprofiler", "Lagrede ladninger med ballistic data"),
            ("rifle_performance", "📊", "Rifle Performance", "Cold bore, barrel tracking, analyse"),
        ]
    ...
    def _add_category_compact(self, layout, title: str):
        ...
        header = QLabel(title)
        header.setStyleSheet("font-size:14px; font-weight:700; color: #ffd700; margin-top:12px; margin-bottom:6px;")
        layout.addWidget(header)
    
    def _create_list_button(self, workflow_id: str, icon: str, title: str, description: str):
        ...
        btn.clicked.connect(lambda _, wf_id=workflow_id: self.workflow_selected.emit(wf_id))
    ...
    def _add_category(self, layout: QVBoxLayout, title: str, description: str):
        ...
        title_label = QLabel(title)
        title_label.setStyleSheet("font-size: 16px; font-weight: bold; color: #2c3e50; background: transparent; border: none;")
        category_layout.addWidget(title_label)
        
        desc_label = QLabel(description)
        desc_label.setStyleSheet("font-size: 12px; color: #7f8c8d; background: transparent; border: none;")
    ...
    def update_mode_description(self):
        ...
        if self.mode_manager.is_beginner():
            self.mode_description.setText("Full tooltips, wizards, confirmation dialogs")
        else:
            self.mode_description.setText("Minimal UI, keyboard shortcuts, direct access")
