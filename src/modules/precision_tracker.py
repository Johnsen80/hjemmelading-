...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                            ...
                            QTableWidgetItem, QTabWidget, QScrollArea, QDateEdit)
...
class PrecisionTracker(QWidget):
    ...
    def init_ui(self):
        ...
        subtitle = QLabel("Analyser sammenhenger mellom vær, ammunisjon og presisjon over tid")
    ...
    def create_overview_tab(self):
        ...
        self.detail_table.setHorizontalHeaderLabels([
            "Dato", "Rifle", "Ammunisjon", "Gruppe (mm)", 
            "MOA", "Temp °C", "Vind m/s"
        ])
    ...
    def update_detail_table(self, data):
        ...
        for i, row in enumerate(data):
            date, rifle, ammo, best_group, avg_group, temp, wind, humidity, distance = row
            ...
            self.detail_table.setItem(i, 3, QTableWidgetItem(f"{best_group:.1f}" if best_group else "N/A"))
            self.detail_table.setItem(i, 4, QTableWidgetItem(moa))
            self.detail_table.setItem(i, 5, QTableWidgetItem(f"{temp:.0f}" if temp else "N/A"))
            self.detail_table.setItem(i, 6, QTableWidgetItem(f"{wind:.1f}" if wind else "N/A"))
    
    def update_weather_analysis(self, data):
        """Oppdaterer væranalyse"""
        # Filtrer ut data med temperatur og vind
        temp_data = [(row[5], row[3]) for row in data if row[5] is not None and row[3] is not None]
        wind_data = [(row[6], row[3]) for row in data if row[6] is not None and row[3] is not None]
...
Gjennomsnittlig gruppe: {best_avg:.1f} mm</p>
    ...
    def update_trend_analysis(self, data):
        ...
        if len(moving_avg) >= 2:
            start_avg = sum(moving_avg[:3]) / 3 if len(moving_avg) >= 3 else moving_avg[0]
            end_avg = sum(moving_avg[-3:]) / 3 if len(moving_avg) >= 3 else moving_avg[-1]
...
Siste økter: {end_avg:.1f} mm gjennomsnitt</p>
        ...
        # Tegn trend-graf
        dates = [datetime.strptime(row[0], "%Y-%m-%d") for row in data if row[3] is not None]
        ...
        if len(moving_avg) > 0:
            ...
            ax.plot(ma_dates, moving_avg, 'r-', linewidth=2, label=f'{window}-økters gj.snitt')
    ...
    def update_alerts(self, data):
        ...
        for i, (severity, alert_type, rifle_ammo, description) in enumerate(alerts):
            ...
            color = QColor(255, 200, 200) if "HØY" in severity else QColor(255, 255, 200)
            for col in range(4):
