...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                            QPushButton, QGroupBox, QGridLayout, QScrollArea, QComboBox, QTableWidget, QTableWidgetItem, QLineEdit, QTextEdit, QCheckBox, QMessageBox, QDoubleSpinBox, QListWidget, QDialog, QFileDialog, QSpinBox)
...
class Dashboard(QWidget):
    def create_ai_tips_widget(self):
        ...
        self.ai_input = QLineEdit(); self.ai_input.setPlaceholderText("Spør AI om ammo, batch, test...")
        ...
        if best:
            layout.addWidget(QLabel(f"Beste ammo: {best['ammo']} batch {best['batch']} ({best['group']} mm samling)"))
        return widget
    ...
    def get_ai_tip(self):
        ...
        if not question:
            self.ai_output.setText("Skriv inn et spørsmål!"); return
        if "presisjon" in question.lower():
            best = self.get_best_ammo()
            if best:
                self.ai_output.setText(f"Beste presisjon: {best['ammo']} batch {best['batch']} med samling {best['group']} mm.")
            else:
                self.ai_output.setText("Ingen testdata for presisjon.")
        elif "hastighet" in question.lower():
            fastest = sorted(lots, key=lambda e: float(e.get("velocity", 0)), reverse=True)[:1]
            if fastest:
                self.ai_output.setText(f"Høyeste hastighet: {fastest[0]['ammo']} batch {fastest[0]['batch']} med {fastest[0]['velocity']} fps.")
            else:
                self.ai_output.setText("Ingen testdata for hastighet.")
        elif "anbefal" in question.lower() or "match" in question.lower():
            best = self.get_best_ammo()
            if best:
                self.ai_output.setText(f"Anbefalt ammo: {best['ammo']} batch {best['batch']} (samling {best['group']} mm)")
            else:
                self.ai_output.setText("Ingen testdata for anbefaling.")
        else:
            self.ai_output.setText("AI: Spør om presisjon, hastighet, anbefaling eller match!")
    ...
    def load_dashboard_profile(self):
        ...
        fname, _ = QFileDialog.getOpenFileName(self, "Last dashboard-oppsett", "", "JSON-filer (*.json)")
        if fname:
    ...
    def create_quick_stats_widget(self):
        ...
        best_group = min([float(e.get('group_size_mm', 9999)) for e in lots if e.get('group_size_mm')], default=None)
        avg_moa = round(sum([float(e.get('moa', 0)) for e in lots if e.get('moa')]) / max(1, len([e for e in lots if e.get('moa')])), 2)
        total_hits = sum([int(e.get('shot_count', 0)) for e in lots if e.get('shot_count')])
        layout.addWidget(QLabel(f"Beste gruppe: {best_group if best_group is not None else '-'} mm"))
    ...
    def create_ammo_comparison_widget(self):
        ...
        for e in lots:
            ...
            if e.get('group_size_mm'): ammo_stats[ammo]['groups'].append(float(e['group_size_mm']))
            if e.get('moa'): ammo_stats[ammo]['moas'].append(float(e['moa']))
        table = QTableWidget()
        ...
        for i, (ammo, stats) in enumerate(ammo_stats.items()):
            avg_group = round(sum(stats['groups']) / max(1, len(stats['groups'])), 2) if stats['groups'] else '-'
            avg_moa = round(sum(stats['moas']) / max(1, len(stats['moas'])), 2) if stats['moas'] else '-'
    ...
    def create_batch_analysis_widget(self):
        ...
        table.setHorizontalHeaderLabels(["Batch", "Ammo", "Gruppe (mm)", "MOA", "Treff", "Bilde"])
        ...
        for i, e in enumerate(lots):
            ...
            stats_layout.addWidget(self.create_stat_card(tr('dashboard_rifles', self.current_language), self.count_rifles()))
            stats_layout.addWidget(self.create_stat_card(tr('dashboard_ammo_profiles', self.current_language), self.count_ammo_profiles()))
            stats_layout.addWidget(self.create_stat_card(tr('dashboard_ladder_tests', self.current_language), self.count_ladder_tests()))
            stats_layout.addWidget(self.create_stat_card(tr('dashboard_loading_sessions', self.current_language), self.count_loading_sessions()))
    ...
    def create_image_analysis_widget(self):
        ...
        lots = getattr(self, 'test_history', [])
        analyzed = [e for e in lots if e.get('group_size_mm') or e.get('moa') or e.get('shot_count')]
        if not analyzed:
            layout.addWidget(QLabel("Ingen analyserte bilder funnet i testhistorikk."))
            return widget
        table = QTableWidget()
        table.setColumnCount(5)
        table.setHorizontalHeaderLabels(["Ammo", "Batch", "Gruppe (mm)", "MOA", "Treff"])
        ...
        if img_path:
            img_label = QLabel()
            img_label.setPixmap(QPixmap(img_path).scaledToWidth(200, Qt.TransformationMode.SmoothTransformation))
    ...
    def create_inventory_status(self):
        ...
        low_powder = [p for p in powders if p['quantity_grams'] and p['quantity_grams'] < 500]
        ...
        if not low_powder and not low_bullets and not low_primers and not worn_cases:
            ...
            # Advarsler
            if low_powder:
                warning = QLabel(f"⚠️ <b>{len(low_powder)} krutt</b> har lavt lager (< 500g)")
                warning.setStyleSheet("color: orange;")
                layout.addWidget(warning)
            
            if low_bullets:
                warning = QLabel(f"⚠️ <b>{len(low_bullets)} kuler</b> har lavt lager (< 100 stk)")
                warning.setStyleSheet("color: orange;")
                layout.addWidget(warning)
            
            if low_primers:
                warning = QLabel(f"⚠️ <b>{len(low_primers)} tennhetter</b> har lavt lager (< 100 stk)")
                warning.setStyleSheet("color: orange;")
                layout.addWidget(warning)
            
            if worn_cases:
                warning = QLabel(f"⚠️ <b>{len(worn_cases)} hylser</b> er brukt > 5 ganger (vurder utsortering)")
                warning.setStyleSheet("color: orange;")
                layout.addWidget(warning)
        ...
        if not loading_sessions and not shooting_sessions and not ladder_tests:
            ...
            if loading_sessions:
                ...
                for session in loading_sessions:
                    ...
                    item = QLabel(f"  • {session['date']}: {session['quantity']} stk {ammo_name}")
            ...
            if shooting_sessions:
                ...
                for session in shooting_sessions:
                    ...
                    item = QLabel(f"  • {session['date']}: {rifle_name} - {session['rounds_fired']} skudd")
                    layout.addWidget(item)
    ...
    def create_quick_tips(self):
        ...
        tip_label.setStyleSheet("font-size: 11pt; padding: 10px; background-color: #ffffcc; border-radius: 5px;")
...
class AmmoTestReport:
    ...
    def log_test(self, lot_number, manufacturer, model, caliber, velocity_list, group_size_list, case_spec, bullet_spec, powder_spec, primer_spec, weapon_weight, notes="", image_path=None):
    ...
    def generate_report(self, test_id, report_type="private"):
        ...
        tests = self.db.execute_query("SELECT * FROM ammo_test_reports WHERE test_id = ?", (test_id,))
        if not tests:
        ...
        bullet_weight = float(bullet_spec.get("weight_grains", 0)) * 0.0648  # grains til gram
        powder_weight = float(powder_spec.get("weight_grains", 0)) * 0.0648
        velocity = float(velocities[0]) if velocities else 0
        energy = self.calculate_energy(velocity, bullet_weight)
        recoil = self.calculate_recoil(bullet_weight, powder_weight, velocity, weapon_weight)
        fig, ax = plt.subplots()
        ...
        if report_type == "store":
            ...
            plt.figtext(0.5, 0.01, tr("store_report_footer", self.language), ha="center", fontsize=10)
            ...
            plt.figtext(0.5, 0.01, tr("supplier_report_footer", self.language), ha="center", fontsize=10)
        else:
        ...
        plt.figtext(0.1, 0.01, f"Anslagsenergi: {energy:.1f} J | Rekyl: {recoil:.2f} J", ha="left", fontsize=10)
    ...
    def analyze_differences(self, current_test, previous_tests):
        ...
        results["group_size_diff"] = curr_group - np.mean(prev_groups) if prev_groups else 0
        results["trend"] = "Bedre" if results["group_size_diff"] < 0 else "Dårligere"
        return results
    def generate_comparison_report(self, lot_number, caliber, manufacturer, model, report_type="private"):
        """Genererer rapport med sammenligning og analyse av tidligere tester"""
        current_test, previous_tests = self.compare_previous_tests(lot_number, caliber, manufacturer, model)
        differences = self.analyze_differences(current_test, previous_tests)
...
class SystemWeaponManager(QWidget):
    ...
    def update_barrel_table(self):
        ...
        barrels = self.db.execute_query("SELECT * FROM barrel_profiles WHERE id IN (SELECT barrel_profile_id FROM rifles WHERE id = ?)", (rifle_id,))
...
class AmmoTestReportUI(QWidget):
    ...
    def add_image(self):
        ...
        from PyQt6.QtWidgets import QFileDialog
        file_path, _ = QFileDialog.getOpenFileName(self, tr("select_image", self.language), "", "Images (*.png *.jpg *.jpeg)")
        if file_path:
            self.image_path = file_path
    def save_test(self):
        # Lagre alle data til AmmoTestReport
        velocity_list = [float(v) for v in self.velocity_input.text().split(",") if v.strip()]
        group_size_list = [float(g) for g in self.group_size_input.text().split(",") if g.strip()]
...
class CaseBatchPrepUI(QWidget):
    ...
    def save_batch(self):
        ...
        QMessageBox.information(self, tr("prep_advice_title", self.language), "\n".join(advice))
...
class WeaponProfileWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.open_demo_btn.setToolTip("Åpne demo-våpenprofilen i redigeringsdialog (hvis tilgjengelig)")
        ...
        self.display_options_file = os.path.join(os.path.expanduser('~'), 'weapon_display_options.json')
    ...
    def update_barrel_info(self):
        ...
        barrel = self.db.execute_query("SELECT * FROM barrel_profiles WHERE id IN (SELECT barrel_profile_id FROM rifles WHERE id = ?)", (rifle_id,))
        if barrel:
            barrel = barrel[0]
            self.barrel_info.setText(f"Navn: {barrel['name']}\nProfil: {barrel['category']}\nKaliber: {barrel['typical_calibers']}")
        else:
    ...
    def update_optics_list(self, rifle_id):
        optics = self.db.execute_query("SELECT * FROM optics WHERE id IN (SELECT optic_id FROM rifle_optics WHERE rifle_id = ?)", (rifle_id,))
    ...
    def edit_current_profile(self):
        rifle_id = self.rifle_select.currentData()
        if not rifle_id:
            QMessageBox.information(self, "Ingen valgt", "Velg et våpen å redigere først.")
            return
    ...
    def delete_current_profile(self):
        rifle_id = self.rifle_select.currentData()
        if not rifle_id:
            QMessageBox.information(self, "Ingen valgt", "Velg et våpen å slette først.")
            return
    ...
    def open_demo_profile(self):
        ...
        demo = self.db.execute_query("SELECT id FROM rifles WHERE name LIKE ? LIMIT 1", ("%VALKYRIE Demo%",))
        if not demo:
            demo = self.db.execute_query("SELECT id FROM rifles WHERE name LIKE ? LIMIT 1", ("%Demo%",))
        if not demo:
            QMessageBox.information(self, "Demo ikke funnet", "Ingen demo-våpen funnet i databasen. Kjør scripts/insert_demo_weapon.py først.")
            return
        rifle_id = demo[0]['id']
        dlg = WeaponProfileDialog(db=self.db, rifle_id=rifle_id, parent=self)
        if dlg.exec() == QDialog.DialogCode.Accepted:
    ...
    def toggle_simple_mode(self):
        ...
        if simple:
            self.ballistics_result.setText("Ballistisk analyse: Klikktabell og grunnleggende data")
        else:
            self.show_ballistics()
...
class AIChatDialog(QDialog):
    ...
    def ask_ai(self):
        ...
        self.answer_label.setText(f"AI svar: (placeholder)")
        pass
...
class ReportWidget(QWidget):
    ...
    def export_report(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, f"Lagre {filetype}", "", f"{filetype} Files (*.{filetype.lower()})")
        if not file_path:
            return
        selected = [k for k, cb in self.content_options.items() if cb.isChecked()]
        data = {'Avstand': [100,200,300], 'Treffgruppe': [30,25,20], 'Hastighet': [820,800,780]}
        fallback = False
        if filetype == "PDF":
            fig = plt.figure()
            ...
            if importlib.util.find_spec("docx") is None:
                QMessageBox.warning(self, "Word ikke tilgjengelig", "Word-eksport krever python-docx. Eksporterer som PDF i stedet.")
                filetype = "PDF"
                fallback = True
            else:
                from docx import Document
            ...
            if importlib.util.find_spec("pandas") is None:
                QMessageBox.warning(self, "Excel ikke tilgjengelig", "Excel-eksport krever pandas. Eksporterer som PDF i stedet.")
                filetype = "PDF"
                fallback = True
            else:
                import pandas as pd
    ...
    def export_pdf(self):
        file_path, _ = QFileDialog.getSaveFileName(self, "Lagre PDF", "", "PDF Files (*.pdf)")
        if not file_path:
            return
...
class OnboardingWidget(QWidget):
    ...
    def show_help(self):
        QMessageBox.information(self, "Hjelpetekst", "Her finner du tips og forklaringer til hvert steg. Du kan også bruke AI chat for mer hjelp.")
