...
class BrassSelectionPage(QWizardPage):
    ...
    def validatePage(self):
        ...
        if self.existing_radio.isChecked():
            ...
            QMessageBox.information(self, "Batch Created", f"Created brass batch: {batch_data['batch_name']}")
...
class BulletSelectionPage(QWizardPage):
    ...
    def on_bullet_changed(self, index):
        ...
        if bullet and isinstance(bullet, dict):
            ...
            self.bullet_bc_label.setText(f"{bullet.get('bc_g7', 0):.3f}" if bullet.get('bc_g7') else "-")
...
class LoadDataPage(QWizardPage):
    ...
    def validatePage(self):
        ...
        if self.min_charge_spin.value() >= self.max_charge_spin.value():
            QMessageBox.warning(self, "Invalid Range", "Minimum charge must be less than maximum charge.")
            return False
...
class AIPredictionPage(QWizardPage):
    ...
    def calculate_prediction(self):
        ...
        test_charges = [min_charge, (min_charge + mid_charge) / 2, mid_charge, 
                       (mid_charge + max_charge) / 2, max_charge]
        
        ballistics_results = []
        for charge in test_charges:
            calc = engine.calculate_load(
                rifle['id'], bullet.get('id'), powder.get('id'),
                charge, coal_mm, cbto_mm
            )
        ...
        if similar_loads:
            valid_charges = [l['charge_weight_grains'] for l in similar_loads if l.get('charge_weight_grains')]
            valid_moas = [l['average_moa'] for l in similar_loads if l.get('average_moa')]
        ...
        if optimal:
            ...
            for b in ballistics_results:
                safety_color = '#27ae60' if b['safety_margin_percent'] > 20 else '#e67e22' if b['safety_margin_percent'] > 10 else '#e74c3c'
    ...
    def create_fallback_prediction(self, html, params):
        ...
        self.wizard.prediction_data = {
            ...
            'expected_sd': None,
            'confidence': 0.3,
            'test_charges': [min_charge, min_charge + step, mid_charge, min_charge + 3*step, max_charge]
        }
...
class BatchCreationPage(QWizardPage):
    def __init__(self, wizard):
        ...
        layout = QVBoxLayout()
        # ...existing code...
        # Print batch label-knapp
        self.print_label_btn = QPushButton("🖨️ Print Batch Label")
        self.print_label_btn.clicked.connect(self.print_batch_label)
        self.print_label_btn.setStyleSheet("background: #16a085; color: white; font-weight: bold; padding: 12px; font-size: 12pt;")
        layout.addWidget(self.print_label_btn)
        # ...existing code...

    def print_batch_label(self):
        """Generer og vis enkel batch label for utskrift"""
        test_charges = self.wizard.prediction_data.get('test_charges', [])
        bullet = self.wizard.bullet_data
        powder = self.wizard.powder_data
        label_html = "<h2 style='color:#16a085;'>Batch Labels</h2>"
        for i, charge in enumerate(test_charges, 1):
            batch_num = f"{self.batch_prefix_input.text()}-{i:03d}"
            label_html += f"<div style='border:2px dashed #16a085; margin:12px; padding:12px; width:350px; font-size:14pt;'>"
            label_html += f"<b>Batch:</b> {batch_num}<br>"
            label_html += f"<b>Kule:</b> {bullet.get('manufacturer','')} {bullet.get('name','')}<br>"
            label_html += f"<b>Vekt:</b> {bullet.get('weight_grains','-')} gr<br>"
            label_html += f"<b>Krutt:</b> {powder.get('manufacturer','')} {powder.get('name','')}<br>"
            label_html += f"<b>Ladning:</b> {charge:.1f} gr"
            label_html += "</div>"
        dlg = QDialog(self)
        dlg.setWindowTitle("Batch Labels for Print")
        dlg.resize(400, 600)
        vbox = QVBoxLayout()
        text = QTextEdit()
    ...
    def __init__(self, wizard):
        ...
        self.create_btn.setStyleSheet("background: #27ae60; color: white; font-weight: bold; padding: 12px; font-size: 12pt;")
        ...
        self.print_btn.setStyleSheet("background: #2980b9; color: white; font-weight: bold; padding: 12px; font-size: 12pt;")
    ...
    def show_summary(self):
        ...
        """.format(len(test_charges) * shots_per_charge)
