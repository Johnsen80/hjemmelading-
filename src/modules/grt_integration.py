...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                            ...
                            QSpinBox, QTabWidget, QFileDialog, QProgressBar)
...
class GRTIntegration(QWidget):
    ...
    def init_ui(self):
        ...
        import_btn.setStyleSheet("background-color: #2196F3; color: white; font-weight: bold;")
        ...
        self.grt_table.setHorizontalHeaderLabels([
            "Ammunisjonsprofil", "GRT Hastighet", "Faktisk Hastighet", "Diff",
            "Max Trykk (PSI)", "Fylling %", "Status", "Importert"
        ])
    ...
    def load_grt_profiles(self):
        ...
        for i, profile in enumerate(profiles):
            ...
            # GRT hastighet
            grt_vel = f"{profile['predicted_velocity']:.0f}" if profile['predicted_velocity'] else "-"
            self.grt_table.setItem(i, 1, QTableWidgetItem(grt_vel))
            
            # Faktisk hastighet
            actual_vel = f"{profile['velocity_fps']:.0f}" if profile['velocity_fps'] else "-"
            ...
            # Max trykk
            pressure = f"{profile['max_pressure_psi']:.0f}" if profile['max_pressure_psi'] else "-"
            ...
            # Fylling %
            fill = f"{profile['case_fill_percent']:.1f}%" if profile['case_fill_percent'] else "-"
    ...
    def export_to_grt(self):
        ...
        if not profiles:
            QMessageBox.warning(self, "Ingen profiler", "Du har ingen ammunisjonsprofiler å eksportere!")
            return
        ...
        for profile in profiles:
            # Hent komponenter
            bullet = self.db.get_by_id("bullets", profile['bullet_id']) if profile['bullet_id'] else None
            powder = self.db.get_by_id("powder", profile['powder_id']) if profile['powder_id'] else None
            primer = self.db.get_by_id("primers", profile['primer_id']) if profile['primer_id'] else None
            case_data = self.db.get_by_id("cases", profile['case_id']) if profile['case_id'] else None
                ...
                with open(file_path, 'w', newline='', encoding='utf-8') as f:
                    if grt_export:
                        writer = csv.DictWriter(f, fieldnames=[
                            'profile_name', 'caliber', 'bullet_weight', 'powder_name',
                            'powder_charge', 'coal', 'actual_velocity'
                        ])
                        writer.writeheader()
...
class GRTImportDialog(QDialog):
    ...
    def parse_and_preview(self, file_path):
        ...
        try:
            if file_path.endswith('.json'):
                ...
                QMessageBox.warning(self, "Format ikke støttet",
            ...
            if not isinstance(data, list):
                QMessageBox.warning(self, "Ugyldig format",
                    "Forventet en liste med profiler.")
                return
            ...
            QMessageBox.critical(self, "Parse-feil", f"Kunne ikke lese filen:\n{str(e)}")
    ...
    def find_matching_profile(self, grt_item):
        ...
        if "eksakt" in strategy:
            ...
            for profile in profiles:
                if (profile['caliber'] == grt_caliber and
                    abs(profile['bullet_weight'] - grt_bullet_weight) < 1 and
                    abs(profile['powder_charge'] - grt_powder_charge) < 0.5):
                    return profile
        ...
        for item in self.grt_data:
            ...
            if match:
                ...
                grt_record = {
                    ...
                    'predicted_velocity': item.get('velocity_fps', item.get('predicted_velocity')),
                    'max_pressure_psi': item.get('max_pressure_psi', item.get('pressure')),
                    'case_fill_percent': item.get('case_fill_percent', item.get('fill_ratio')),
...
class GRTAnalysisDialog(QDialog):
    ...
    def create_velocity_tab(self):
        ...
        for i, profile in enumerate(self.profiles):
            ...
            table.setItem(i, 1, QTableWidgetItem(f"{profile['predicted_velocity']:.0f} fps"))
    ...
    def create_pressure_tab(self):
        ...
        for i, profile in enumerate(self.profiles):
            ...
            table.setItem(i, 2, QTableWidgetItem(f"{profile['case_fill_percent']:.1f}%"))
