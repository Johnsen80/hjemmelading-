            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    try:
                        ...
                        temp = float(row.get('Temperature', 0)) if row.get('Temperature') else None
                        
                        if velocity > 0:  # Valid reading
                            readings.append(ChronographReading(
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    try:
                        ...
                        velocity_str = row.get('Velocity (fps)', row.get('Velocity', '0'))
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for line in lines:
                    ...
                    velocity_match = re.search(r'(\d+\.?\d*)\s*fps', line, re.IGNORECASE)
                    if velocity_match:
            ...
            session_name = os.path.basename(file_path).replace('.txt', '').replace('.csv', '')
            date = datetime.now().strftime("%Y-%m-%d")
...
class ChronographImporter(QWidget):
    ...
    def init_ui(self):
        ...
        self.device_combo.addItems(["LabRadar (CSV)", "Garmin Xero (CSV)", "MagnetoSpeed (TXT/CSV)"])
        ...
        import_btn.setStyleSheet("font-size: 12pt; font-weight: bold; background-color: #4CAF50; color: white;")
        ...
        self.shots_table.setHorizontalHeaderLabels(["Skudd #", "Hastighet (fps)", "Tid", "Notater"])
...
"""
        ...
        for idx, reading in enumerate(session.raw_data):
            ...
            self.shots_table.setItem(idx, 1, QTableWidgetItem(f"{reading.velocity_fps:.1f}"))
