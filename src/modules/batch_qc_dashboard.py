...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton,
                             ...
                             QTextEdit, QMessageBox, QLineEdit, QProgressBar,
...
class BatchQCDashboard(QWidget):
    ...
    def init_ui(self):
        ...
        self.label_outliers.setStyleSheet("font-size: 14px; font-weight: bold; color: #e74c3c;")
        stats_layout.addWidget(self.label_outliers)
        
        self.label_status = QLabel("Status: Venter")
        self.label_status.setStyleSheet("font-size: 14px; font-weight: bold; color: #7f8c8d;")
        ...
        self.table_measurements.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
    ...
    def start_batch(self):
        ...
        self.label_status.setStyleSheet("font-size: 14px; font-weight: bold; color: #27ae60;")
    ...
    def add_measurement(self, measurement_type: str):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            self.table_measurements.setItem(row, 0, QTableWidgetItem(str(data['patron_number'])))
            self.table_measurements.setItem(row, 1, QTableWidgetItem(type_names[measurement_type]))
            self.table_measurements.setItem(row, 2, QTableWidgetItem(f"{data['value']:.3f}"))
            self.table_measurements.setItem(row, 3, QTableWidgetItem(f"{target:.3f}" if target else "N/A"))
            ...
            total_outliers = sum(1 for v in self.qc_data.values() for m in v if m['outlier'])
        ...
        for i, (mtype, stat) in enumerate(stats.items()):
            ...
            if outliers:
                for outlier in outliers:
                    ax.axvline(outlier, color='red', linestyle='--', linewidth=2, alpha=0.7)
    ...
    def generate_qc_report(self, stats: Dict) -> str:
        ...
        if 'charge_weight' in stats:
            ...
            if charge_std < 0.05:
                ...
                html += "👍 <b>Good!</b> Din konsistens er akseptabel (Hornady: ±0.1gr)<br>"
            else:
                html += "⚠️ <b>Poor!</b> Fabrikk-ammo er bedre. Sjekk powder thrower!<br>"
