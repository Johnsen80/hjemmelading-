...
from src.modules.interactive_features import InteractiveVelocityGraph, InteractivePowderChargeSlider
from src.modules.workflow_state import WorkflowStateManager
...
class LadderTestLab(QWidget):
    ...
    def init_ui(self):
        ...
        self.tests_table.setHorizontalHeaderLabels([
            "Navn", "Dato", "Rifle", "Kaliber", "Ladning (gr)", "Avstand", "Resultater"
        ])
    ...
    def new_ladder_test(self):
        ...
        dialog = LadderTestDialog(self, rifles=rifles, powders=powders,
                                 bullets=bullets, primers=primers, cases=cases)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            QMessageBox.information(self, "Test opprettet", 
...
class LadderTestDialog(QDialog):
    ...
    def __init__(self, parent=None, rifles=None, powders=None, bullets=None, 
                 primers=None, cases=None):
...
class TestResultsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.results_table.setHorizontalHeaderLabels([
            "Ladning (gr)", "V1 (fps)", "V2 (fps)", "V3 (fps)", "ES", "SD",
            "Gruppe (mm)", "Notater"
        ])
    ...
    def get_results(self):
        ...
        for i in range(self.results_table.rowCount()):
            ...
            notes = self.results_table.item(i, 7).text() if self.results_table.item(i, 7) else ""
...
class AnalysisWindow(QDialog):
    ...
    def create_live_testing_tab(self):
        ...
        # Info banner
        info_banner = QLabel("🔴 <b>LIVE TESTING MODE</b> - Enter data as you shoot, see graphs update in real-time!")
        ...
        self.live_shot_log.setHorizontalHeaderLabels(["#", "Charge (gr)", "Velocity (fps)"])
    ...
    def restore_live_test_state(self, state_data):
        ...
        if 'shots' in state_data:
            ...
            for shot in state_data['shots']:
                ...
                self.live_shot_log.setItem(row, 0, QTableWidgetItem(f"{self.shot_counter}"))
                self.live_shot_log.setItem(row, 1, QTableWidgetItem(f"{shot['charge']:.1f}"))
                self.live_shot_log.setItem(row, 2, QTableWidgetItem(f"{shot['velocity']}"))
        
        if 'last_charge' in state_data:
            # Find and select the charge in combo
            for i in range(self.live_charge_combo.count()):
                if abs(self.live_charge_combo.itemData(i) - state_data['last_charge']) < 0.01:
    ...
    def create_data_tab(self):
        ...
        for i, result in enumerate(self.results):
            ...
            group_mm = f"{result['group_size_mm']:.1f}" if result['group_size_mm'] else "-"
            table.setItem(i, 4, QTableWidgetItem(group_mm))
            
            # Beregn MOA
            if result['group_size_mm'] and self.test['distance_meters']:
                moa = (result['group_size_mm'] / 10) / (self.test['distance_meters'] / 100) / 2.908
    ...
    def create_graphs_tab(self):
        ...
        # Hent data
        charges = [r['charge_weight'] for r in self.results]
    ...
    def create_recommendation_tab(self):
        ...
        for result in self.results:
            if result['velocity_sd']:
                if best_sd is None or result['velocity_sd'] < self.results[best_sd]['velocity_sd']:
                    best_sd = self.results.index(result)
            
            if result['group_size_mm']:
                if best_group is None or result['group_size_mm'] < self.results[best_group]['group_size_mm']:
        ...
        if best_group is not None:
            ...
            moa = (r['group_size_mm'] / 10) / (self.test['distance_meters'] / 100) / 2.908 if self.test['distance_meters'] else 0
            rec_text += f"""
