...
from PyQt6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QGridLayout, QFrame, QTextEdit
)
...
class ModernDashboard(QWidget):
    def __init__(self):
        ...
        cards = [
            ("⚡ Lade-modul", "Start lading, se grafer og AI-anbefalinger", self.open_loading_workspace),
            ("🔬 Ballistikk-simulator", "Avansert simulering av trykk og velocity", self.open_ballistics_simulator),
            ("📦 Batch QC", "Batch management og analyse", self.open_batch_qc),
            ("🤖 AI Chat", "Få AI-hjelp og forklaringer", self.open_ai_chat),
            ("📊 Historikk & Rapporter", "Se og eksporter testresultater", self.open_history_reports),
            ("⚙️ Innstillinger", "Tilpass programmet", self.open_settings)
        ]
        for i, (title, desc, func) in enumerate(cards):
            card = QFrame()
            card.setStyleSheet("background: #2c2f33; border-radius: 12px; padding: 30px; border: 2px solid #e67e22;")
            vbox = QVBoxLayout()
            label = QLabel(title)
            ...
            btn.setStyleSheet("background: #e67e22; color: #23272a; font-weight: bold; padding: 10px 30px; border-radius: 8px;")
        ...
        status_box = QFrame()
        status_box.setStyleSheet("background: #2c2f33; border-radius: 12px; padding: 20px; margin-top: 40px;")
        ...
        ai_box.setText("AI: Test 42.3gr H4350, seating 70.8mm. Unngå trykk over 59k PSI. Group size: 0.65 MOA.")
