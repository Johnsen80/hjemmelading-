...
class LiveVelocityGraph(BaseCanvas):
    ...
    def _update_plot(self):
        ...
        # Scatter plot
        self.ax.scatter(self.data_x, self.data_y, s=100, c='#3498db',
                       edgecolors='#2c3e50', linewidths=2, zorder=3,
                       label='Shots')
    ...
    def _detect_nodes(self):
        ...
        if nodes:
            ...
            self.ax.scatter(node_x, node_y, s=200, facecolors='none',
...
class LiveGroupOverlay(BaseCanvas):
    ...
    def _update_plot(self):
        ...
        for idx, charge in enumerate(charge_list):
            ...
            # Plot shots
            self.ax.scatter(x_vals, y_vals, s=80, c=color, alpha=0.7,
            ...
            if len(shots) >= 3:
                ...
                # Add group size text
                self.ax.text(center_x, center_y + radius + 0.2,
                           f'{radius*2:.3f}"', ha='center', fontsize=9,
...
class LiveStatisticsDisplay(QWidget):
    ...
    def _create_stat_label(self, name: str, value: str, color: str) -> QWidget:
        ...
        name_label.setStyleSheet("color: white; font-size: 12px; background: transparent;")
        name_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(name_label)
        
        value_label = QLabel(value)
        value_label.setStyleSheet("color: white; font-size: 24px; font-weight: bold; background: transparent;")
...
class LiveHistogram(BaseCanvas):
    ...
    def _update_plot(self):
        ...
        # Histogram
        n, bins, patches = self.ax.hist(self.data, bins=20, color='#3498db',
        ...
        # Target line
        if self.target:
            self.ax.axvline(self.target, color='#27ae60', linestyle='--',
                          linewidth=2, label=f'Target: {self.target}{self.unit}')
        
        # Tolerance lines
        if self.tolerance:
            self.ax.axvline(self.target - self.tolerance, color='#f39c12',
        ...
        if len(self.data) > 1:
            ...
            outliers = sum(1 for v in self.data 
                          if abs(v - self.target) > self.tolerance) if self.tolerance else 0
