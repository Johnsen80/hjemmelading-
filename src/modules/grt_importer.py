    ...
    def run(self):
        try:
            ...
            for i, category in enumerate(self.categories):
                self.progress.emit(f"Laster ned {category}...", int((i/len(self.categories))*100))
                time.sleep(1)
...
class GRTImporter(QWidget):
    ...
    def download_from_github(self):
        ...
        if not categories:
            QMessageBox.warning(self, "Ingen valg", "Velg minst én kategori å laste ned!")
            return
    ...
    def import_folder(self):
        ...
        if not files:
            QMessageBox.warning(
                self, 
                "Ingen filer", 
                f"Fant ingen {pattern} filer i den valgte mappen."
            )
            return
