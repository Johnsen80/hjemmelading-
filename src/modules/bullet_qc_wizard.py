...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                            ...
                            QTextEdit, QMessageBox, QProgressBar, QWizard,
                            QWizardPage, QRadioButton, QCheckBox, QSplitter)
...
class QCHistoryDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Title
        title = QLabel(f"<h2>📊 Quality Control History</h2><p><b>{self.bullet_name}</b></p>")
        ...
        table.setHorizontalHeaderLabels([
            "Date", "Lot Number", "Sample Size", "Weight Avg", "Weight SD",
            "Ogive Avg", "Ogive SD", "Quality"
        ])
        ...
        for i, record in enumerate(self.history):
            ...
            table.setItem(i, 3, QTableWidgetItem(f"{stats.get('weight_avg', 0):.2f} gr"))
        ...
        if len(self.history) > 1:
            best_lot = min(self.history, key=lambda x: x['statistics'].get('ogive_sd', 999))
            worst_lot = max(self.history, key=lambda x: x['statistics'].get('ogive_sd', 0))
            ...
            summary.setStyleSheet("background-color: #d5f4e6; padding: 15px; border-radius: 5px; margin-top: 10px;")
    ...
    def save_qc_history(self, results: Dict):
        ...
        if os.path.exists(history_file):
            try:
                ...
                logger.debug("Failed to load QC history file, starting fresh: %s", e, exc_info=True)
                try:
...
class IntroPage(QWizardPage):
    ...
    def __init__(self):
        ...
        self.setSubTitle("Measure your bullets to find the best ones for match shooting!")
...
class SetupPage(QWizardPage):
    ...
    def __init__(self):
        ...
        self.custom_bullet.toggled.connect(lambda: self.custom_name.setEnabled(self.custom_bullet.isChecked()))
        self.custom_bullet.toggled.connect(lambda: self.bullet_combo.setEnabled(not self.custom_bullet.isChecked()))
        ...
        help_label.setStyleSheet("background-color: #e8f4f8; padding: 10px; border-radius: 5px;")
        ...
        help2.setStyleSheet("background-color: #d5f4e6; padding: 8px; border-radius: 5px; margin-top: 10px;")
...
class MeasurementPage(QWizardPage):
    ...
    def __init__(self):
        ...
        self.current_bullet.setStyleSheet("font-size: 16px; font-weight: bold; color: #2c3e50;")
        ...
        self.live_stats.setStyleSheet("background-color: #ecf0f1; padding: 10px; border-radius: 5px;")
    ...
    def add_measurement(self):
        ...
        self.measurements_table.setItem(row, 1, QTableWidgetItem(f"{measurement['weight']:.2f}"))
        self.measurements_table.setItem(row, 2, QTableWidgetItem(f"{measurement['length']:.4f}"))
        self.measurements_table.setItem(row, 3, QTableWidgetItem(f"{measurement['ogive']:.4f}"))
        
        # Update progress
        self.progress_bar.setValue(bullet_num)
        self.progress_label.setText(f"Progress: {bullet_num} / {sample_size} bullets measured")
        ...
        if bullet_num < sample_size:
            ...
            import random
            self.weight_input.setValue(self.weight_input.value() + random.uniform(-0.2, 0.2))
            self.length_input.setValue(self.length_input.value() + random.uniform(-0.002, 0.002))
            self.ogive_input.setValue(self.ogive_input.value() + random.uniform(-0.001, 0.001))
            self.weight_input.setFocus()
            self.weight_input.selectAll()
        else:
...
class ResultsPage(QWizardPage):
    ...
    def __init__(self):
        ...
        """)
        next_steps.setStyleSheet("background-color: #e8f4f8; padding: 15px; border-radius: 5px;")
    ...
    def update_results(self, measurements_dict: Dict):
        ...
        length_es = max(lengths) - min(lengths)
        ...
        # Sorting recommendations
        match_count = sum(1 for m in measurements if abs(m['ogive'] - ogive_avg) <= 0.001)
