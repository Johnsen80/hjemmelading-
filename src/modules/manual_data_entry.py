...
from PyQt6.QtWidgets import (
    QWidget, QVBoxLayout, QLabel, QLineEdit, QPushButton, QComboBox, QDoubleSpinBox, QMessageBox, QSpinBox, QInputDialog
)
from sqlalchemy.orm import Session
from src.database.database import PowderData, BulletData
...
class ManualDataEntryWidget(QWidget):
    ...
    def add_bullet_measurement(self):
        if len(self.bullet_measurements) < self.bullet_measure_count.value():
            value, ok = QInputDialog.getDouble(self, "Kulemåling", "Skriv inn måling (mm eller grain):", 0, 0, 1000, 2)
            if ok:
                self.bullet_measurements.append(value)
                self.bullet_measure_list.setText(f"Målinger: {self.bullet_measurements}")
        else:
            QMessageBox.warning(self, "Maks målinger", "Du har nådd valgt antall målinger.")
    def calc_bullet_stats(self):
        ...
        std = (sum((x - avg) ** 2 for x in self.bullet_measurements) / len(self.bullet_measurements)) ** 0.5
    ...
    def save_powder_stock(self):
        ...
        QMessageBox.information(self, "Lagring", f"Kruttbeholdning lagret: {amount} {unit} ({amount_grain:.0f} grain)")
