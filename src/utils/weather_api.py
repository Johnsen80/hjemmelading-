...
class WeatherAPIService:
    ...
    def get_weather(self, lat: float, lon: float) -> Dict:
        ...
        return {
            'error': True,
            'message': f"Alle vær-kilder feilet:\n" + "\n".join(errors)
        }
    ...
    def _get_yrno(self, lat: float, lon: float) -> Optional[Dict]:
        ...
        headers = {'User-Agent': 'ReloadingWorkshopManager/1.0 (https://github.com/yourusername)'}
    ...
    def set_api_keys(self, openweathermap: str = None, weatherapi: str = None):
        """Setter API-nøkler"""
        if openweathermap:
    ...
    def compare_sources(self, lat: float, lon: float) -> Dict:
        ...
        for provider in self.providers:
            try:
                ...
                logger.debug("Weather provider '%s' failed: %s", provider, e, exc_info=True)
                try:
