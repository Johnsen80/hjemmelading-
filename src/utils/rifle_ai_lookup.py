...
class RifleAILookup:
    ...
    def lookup_rifle(self, rifle_name: str, manufacturer: Optional[str] = None) -> Dict[str, any]:
        """
                ...
                for path in common_paths:
                    try:
                        ...
                        if response.status_code == 200:
                            result = self._parse_page(response.text, mfg_data['patterns'], result)
                            result['sources'].append(url)
                            result['confidence'] = 'medium'
                            return result
                    except Exception as e:
                        logger.debug("AI lookup request failed for %s: %s", url, e, exc_info=True)
                        try:
