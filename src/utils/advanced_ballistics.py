...
class G1DragFunction:
    ...
    MACH_TABLE = [0.0, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 1.0, 1.05, 1.1, 1.2, 1.3, 1.5, 1.7, 2.0, 2.5, 3.0, 4.0, 5.0]
    CD_TABLE = [0.2629, 0.2558, 0.2487, 0.2413, 0.2344, 0.2278, 0.2246, 0.2214, 0.2538, 0.3105, 0.3732, 0.4084, 0.4448, 0.4615, 0.4740, 0.4825, 0.4880, 0.4960, 0.5020]
...
class G7DragFunction:
    ...
    MACH_TABLE = [0.0, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 1.0, 1.05, 1.1, 1.2, 1.3, 1.5, 1.7, 2.0, 2.5, 3.0, 4.0, 5.0]
    CD_TABLE = [0.1198, 0.1197, 0.1196, 0.1194, 0.1193, 0.1194, 0.1194, 0.1194, 0.1252, 0.1396, 0.1581, 0.1709, 0.1891, 0.2029, 0.2190, 0.2400, 0.2550, 0.2800, 0.2980]
...
class AdvancedBallisticsEngine:
    ...
    def calculate_trajectory(
        ...
        zero_angle = self._calculate_zero_angle(
            velocity_fps, bc, zero_distance_m * self.METERS_TO_FEET,
            density_ratio, drag_func
        )
        ...
        while x * self.FEET_TO_METERS <= max_distance_m:
            ...
            if x * self.FEET_TO_METERS >= current_distance_m:
                ...
                drop_cm = -y * self.FEET_TO_METERS * 100  # Negative because y is positive up
            ...
            k2_vx, k2_vy = self._derivatives(vx + k1_vx * dt / 2, vy + k1_vy * dt / 2, v, bc, density_ratio, drag_func)
            k3_vx, k3_vy = self._derivatives(vx + k2_vx * dt / 2, vy + k2_vy * dt / 2, v, bc, density_ratio, drag_func)
            k4_vx, k4_vy = self._derivatives(vx + k3_vx * dt, vy + k3_vy * dt, v, bc, density_ratio, drag_func)
    ...
    def _derivatives(self, vx: float, vy: float, v: float, bc: float, 
                    density_ratio: float, drag_func) -> Tuple[float, float]:
    ...
    def _calculate_zero_angle(self, velocity_fps: float, bc: float, 
                             zero_distance_ft: float, density_ratio: float,
                             drag_func) -> float:
    ...
    def _calculate_drop_at_distance(self, velocity_fps: float, bc: float,
                                   distance_ft: float, angle: float,
                                   density_ratio: float, drag_func) -> float:
    ...
    def _calculate_windage(self, time_s: float, wind_mph: float, 
                          wind_angle_deg: float, velocity_fps: float,
                          bc: float, density_ratio: float, drag_func) -> float:
    ...
    def _calculate_spin_drift(self, distance_m: float, velocity_fps: float,
                            twist_rate: float, twist_direction: str) -> float:
        """
    ...
    def _calculate_coriolis(self, time_s: float, latitude_deg: float,
                           azimuth_deg: float, vx: float, vy: float) -> float:
        """
    ...
    def get_drop_at_distance(self, trajectory: List[TrajectoryPoint], 
                            distance_m: float) -> TrajectoryPoint:
        ...
        for i in range(len(trajectory) - 1):
            if trajectory[i].distance_m <= distance_m <= trajectory[i + 1].distance_m:
                ...
                return TrajectoryPoint(
                    ...
                    velocity_fps=p1.velocity_fps + ratio * (p2.velocity_fps - p1.velocity_fps),
                    ...
                    windage_moa=p1.windage_moa + ratio * (p2.windage_moa - p1.windage_moa),
                    windage_mrad=p1.windage_mrad + ratio * (p2.windage_mrad - p1.windage_mrad),
                    energy_ftlbs=p1.energy_ftlbs + ratio * (p2.energy_ftlbs - p1.energy_ftlbs),
                    momentum=p1.momentum + ratio * (p2.momentum - p1.momentum),
                    drift_spin_cm=p1.drift_spin_cm + ratio * (p2.drift_spin_cm - p1.drift_spin_cm),
                    drift_coriolis_cm=p1.drift_coriolis_cm + ratio * (p2.drift_coriolis_cm - p1.drift_coriolis_cm)
                )
        
        return trajectory[-1] if distance_m > trajectory[-1].distance_m else trajectory[0]
