...
def tr(key, lang="no"):
    """Returnerer oversatt tekst for valgt nøkkel og språk"""
    # Bruker Translations-klassen hvis tilgjengelig, ellers fallback
    try:
        global _translations_instance
        if '_translations_instance' not in globals():
            _translations_instance = Translations()
        _translations_instance.set_language(lang)
        return _translations_instance.get(key)
    except Exception:
        # Fallback
        fallback = {
            'no': {
                'title': "Terrengkart - Geospatial Skyteanalyse",
                'subtitle': "3D terrengvisualisering med høydekurver og ballistisk analyse",
                'reset_positions': "Reset posisjoner",
                'save_location': "Lagre lokasjon",
                'load_location': "Last lokasjon",
                'analyze_shot': "Analyser Skyting",
                'weather': "Vær",
                'select_language': "Velg språk:",
                'ai_coach': "AI Coach",
                'ask_ai': "Spør AI-coach",
                'type_question': "Skriv et spørsmål til AI-coach!",
                'error': "Feil",
                'normal_conditions': "Normale forhold.",
                'position_set': "Posisjoner satt! Klikk 'Analyser Skyting' for å få terrengdata og ballistikk.",
            },
            'en': {
                'title': "Terrain Map - Geospatial Shooting Analysis",
                'subtitle': "3D terrain visualization with elevation contours and ballistic analysis",
                'reset_positions': "Reset positions",
                'save_location': "Save location",
                'load_location': "Load location",
                'analyze_shot': "Analyze Shot",
                'weather': "Weather",
                'select_language': "Select language:",
                'ai_coach': "AI Coach",
                'ask_ai': "Ask AI Coach",
                'type_question': "Type a question for the AI coach!",
                'error': "Error",
                'normal_conditions': "Normal conditions.",
                'position_set': "Positions set! Click 'Analyze Shot' to get terrain and ballistics data.",
            }
        }
        return fallback.get(lang, fallback['no']).get(key, key)
