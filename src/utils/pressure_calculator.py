...
class PressureCalculator:
    ...
    def estimate_pressure_from_velocity(
        ...
        if measured_velocity_fps < manual_min_velocity:
            ...
            velocity_ratio = (measured_velocity_fps - manual_min_velocity) / (manual_max_velocity - manual_min_velocity)
            estimated_psi = manual_min_pressure + (velocity_ratio * (manual_max_pressure - manual_min_pressure))
    ...
    def estimate_pressure_from_charge(
        ...
        charge_ratio = (powder_charge_grains - manual_min_charge) / (manual_max_charge - manual_min_charge)
        
        # Eksponentiell faktor (trykk øker raskere ved høye ladninger)
        pressure_ratio = charge_ratio ** 1.5  # Power 1.5 gir realistic kurve
        
        estimated_psi = manual_min_pressure + (pressure_ratio * (manual_max_pressure - manual_min_pressure))
        return estimated_psi
    
    def calculate_compression_ratio(
        self,
        case_volume_cm3: float,
        powder_volume_cm3: float,
        bullet_volume_cm3: float
    ) -> float:
        """
    ...
    def evaluate_safety(
        ...
        if manual_data and measured_velocity_fps:
            ...
            warnings.append("ADVARSEL: Ingen manual-data tilgjengelig! Estimat er svært usikkert.")
        ...
        if percent_of_max > 100:
            ...
            warnings.append(f"⛔ FARE! Estimert trykk ({estimated_psi:,.0f} PSI) overstiger SAAMI max ({max_saami_psi:,} PSI)!")
        elif percent_of_max > 95:
            safety_rating = 'CAUTION'
            warnings.append(f"⚠️ FORSIKTIG! Ladning er nær SAAMI max ({percent_of_max:.1f}% av max).")
        else:
            safety_rating = 'SAFE'
        ...
        if load_density < 85:
            warnings.append(f"⚠️ Lav load density ({load_density:.1f}%). Kan gi høy ES. Vurder raskere krutt.")
        elif load_density > 105:
            warnings.append(f"⛔ HØYT komprimert ladning ({load_density:.1f}%)! FARE for hylseutvidelse!")
    ...
    def check_load_against_manual(
        ...
        if powder_charge < manual_min:
            return False, f"⚠️ Ladning ({powder_charge:.1f}gr) er UNDER manual minimum ({manual_min:.1f}gr). Kan gi upålitelig tenning."
        elif powder_charge > manual_max:
            return False, f"⛔ FARE! Ladning ({powder_charge:.1f}gr) OVERSTIGER manual maximum ({manual_max:.1f}gr)!"
        elif powder_charge > manual_max * 0.98:
            return True, f"⚠️ FORSIKTIG! Ladning ({powder_charge:.1f}gr) er nær manual max ({manual_max:.1f}gr)."
        else:
