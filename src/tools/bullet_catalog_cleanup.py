...
def _display_name(
    ...
    *,
    ...
    if normalized_display_name and normalized_display_name != original_composed_name:
...
def _variant_rank(value: str, count: int) -> tuple[int, int, int, str]:
    ...
    return (looks_titled, uppercase_letters, count, value)
...
def _normalize_external_ref(
    external_ref: Any, manufacturer: str, name: str, caliber: str, weight_grains: Any
) -> str:
    normalized_ref = _normalize_text(external_ref)
    if not normalized_ref:
        return ""
    parts = normalized_ref.split("|")
    if len(parts) >= 6 and parts[1].lower() == "bullet":
        if parts[0].strip().lower() == LEGACY_MEMORY_DUMP_REF:
            parts[0] = NEUTRAL_MEMORY_DUMP_REF
        parts[2] = manufacturer
        parts[3] = name
        parts[4] = caliber
        parts[5] = _format_numeric_text(weight_grains)
        return "|".join(parts)
    return normalized_ref


def _normalize_raw_json(
    raw_json: Any, manufacturer: str, name: str, caliber: str
) -> str:
    ...
    if "pname" in payload:
        payload["pname"] = name
    if "caliber" in payload:
        payload["caliber"] = caliber
    return json.dumps(payload, ensure_ascii=False)


def clean_bullet_rows(
    ...
    for row in rows:
        ...
        caliber = _normalize_caliber(row.get("caliber"))
        ...
        cleaned["raw_json"] = _normalize_raw_json(
            row.get("raw_json"), manufacturer, name, caliber
        )
        ...
        if _normalize_caliber(original.get("caliber")) != caliber:
            stats["caliber_normalized"] += 1
...
def build_markdown_report(
    ...
    return "\n".join(
        [
            ...
            f"- Caliber normalized: `{stats.get('caliber_normalized', 0)}`",
