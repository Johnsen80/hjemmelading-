...
from .database import PowderData, BulletData
    ...
    with open(file_path, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            db_powder = PowderData(
                ...
                type=row.get('Type'),
                burn_rate=float(row.get('BurnRate', 0)) if row.get('BurnRate') else None,
                energy_density=float(row.get('EnergyDensity', 0)) if row.get('EnergyDensity') else None,
                recommended_charge_min=float(row.get('RecommendedChargeMin', 0)) if row.get('RecommendedChargeMin') else None,
                recommended_charge_max=float(row.get('RecommendedChargeMax', 0)) if row.get('RecommendedChargeMax') else None,
    ...
    with open(file_path, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            db_bullet = BulletData(
                ...
                recommended_twist=float(row.get('RecommendedTwist', 0)) if row.get('RecommendedTwist') else None,
                reference=row.get('Reference')
            )
