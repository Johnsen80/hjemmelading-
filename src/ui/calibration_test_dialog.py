...
class CalibrationTestDialog(QDialog):
    ...
    def __init__(self, parent=None, existing: Optional[Dict[str, Any]] = None, profile: Optional[Dict[str, Any]] = None, persist_callback=None):
    ...
    def init_ui(self) -> None:
        ...
        for i in range(3):
            ...
            velocities_widget.setPlaceholderText("Enter velocities (m/s) separated by commas or newlines, or import CSV")
            ...
            def _parse_and_show():
                ...
                stats = chrono.stats()
                if stats.get("n", 0) > 0:
                    sec["vel_summary"].setText(f"N={stats['n']} mean={stats['mean']:.1f} ES={stats['es']:.1f} SD={stats['sd']:.2f}")
                else:
        ...
        help_btn.clicked.connect(lambda: HelpModal(self, title="Calibration Help").exec())
    ...
    def _make_import_handler(self, velocities_widget: QTextEdit):
        def handler() -> None:
            path, _ = QFileDialog.getOpenFileName(self, "Open chrono CSV", "", "CSV Files (*.csv);;All Files (*)")
            if not path:
                return
            try:
    ...
    def _make_image_handler(self, sec: Dict[str, Any]):
        def handler() -> None:
            path, _ = QFileDialog.getOpenFileName(self, "Open group image", "", "Images (*.png *.jpg *.jpeg);;All Files (*)")
            if not path:
                return
            ...
            try:
                ...
                pix = QPixmap(path)
            lbl = sec["img_label"]
            sec["img_label"].setPixmap(pix.scaled(lbl.width(), lbl.height(), Qt.AspectRatioMode.KeepAspectRatio))
        ...
        def handler() -> None:
            ...
            if dlg.exec():
                ...
                if mm_per_px:
                    ...
                    QMessageBox.information(self, "Calibrated", f"Scale set: {mm_per_px:.6f} mm/pixel")
    ...
    def gather(self) -> Dict[str, Any]:
        ...
        for sec in self.load_sections:
            ...
            vals = []
            for token in velocities_text.replace(";", ",").replace("\n", ",").split(","):
                t = token.strip()
                if not t:
                    continue
                try:
            ...
            load = {
                ...
                "seating_depth_col_mm": float(sec["seating"].text()) if sec["seating"].text().strip() else None,
                "velocities": vals,
    ...
    def _on_analyze(self) -> None:
        ...
        dlg = CalibrationAnalysisDialog(results=results, parent=self, profile=self._profile, persist_callback=self._persist_callback)
        ...
        try:
            ...
            for i, res in enumerate(results):
                ...
                if img_analysis and img_analysis.get("center") and mm_per_px and img_path and active_optic:
                    try:
                        from PyQt6.QtGui import QPixmap
                        ...
                        if not center or not (hasattr(center, '__iter__') and len(center) >= 2):
                            continue
                        ...
                        zero_distance = None
                        try:
                            zero_distance = float(active_optic.get("zero_distance_m")) if active_optic.get("zero_distance_m") else None
                        except Exception:
                            zero_distance = None
                        use_range = zero_distance if zero_distance else 100.0
                        # vertical correction: negate dy_m because pixel y grows downward
                        v_res = compute_clicks_for_offset(-dy_m, use_range, unit=active_optic.get("turret_units", "mil"), click_value=float(active_optic.get("click_value") or 0.1), clicks_per_rev=int(active_optic.get("clicks_per_rev") or 0))
                        # horizontal correction using dx_m
                        h_res = compute_clicks_for_offset(dx_m, use_range, unit=active_optic.get("turret_units", "mil"), click_value=float(active_optic.get("click_value") or 0.1), clicks_per_rev=int(active_optic.get("clicks_per_rev") or 0))
