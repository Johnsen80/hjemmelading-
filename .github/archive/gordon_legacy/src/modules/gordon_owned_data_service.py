...
def build_profile_health_summary(
    ...
    if not parts and has_reference_data:
        parts.append("Har referansedata")
    if not parts:
        parts.append("Ingen helseinfo ennå")
...
def build_profile_trust_map(evidence_summary: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if measured_score:
        notes.append(
            f"Målt grunnlag: {chrono_count} chrono, {accuracy_count} presisjon."
        )
    if simulated_score:
        notes.append("Simulert grunnlag finnes for fart/trykk.")
    if derived_score:
        notes.append("Avledet grunnlag bruker lotter, brass og aktiv analyse.")
    if projectile_level in {"ok", "info"}:
        notes.append(
            "Projectile fit is derived from construction, terminal window, and intended use."
        )
    if assumed_score:
        notes.append("Antatt grunnlag er fortsatt synlig i deler av modellen.")
    ...
    return {
        ...
        "summary": " | ".join(notes[:3]) if notes else "Ingen tillitsdata ennå",
    }
...
def _build_lot_quality_summary(
    ...
    *,
    ...
    if component_type == "bullet":
        ...
        if sample_count >= 10:
            summary["level"] = "ok"
            summary["title"] = "Kulelot er godt målt"
        elif sample_count >= 3:
            summary["level"] = "info"
            summary["title"] = "Kulelot er delvis målt"
        else:
            summary["level"] = "unknown"
            summary["title"] = "Kulelot mangler måleserie"
        note = f"kulelot {summary['lot_number'] or lot_id}: {sample_count} målinger"
    ...
    if component_type == "powder":
        if pressure_watch_count > 0:
            ...
            summary["title"] = "Kruttlot krever oppfølging"
        elif confidence_score >= 0.7 or data_points >= 4:
            summary["level"] = "ok"
            summary["title"] = "Kruttlot er godt kjent"
        elif confidence_score > 0.0 or data_points > 0:
            summary["level"] = "info"
            summary["title"] = "Kruttlot er under læring"
        else:
            summary["level"] = "unknown"
            summary["title"] = "Kruttlot mangler læring"
        summary["note"] = (
            f"kruttlot {summary['lot_number'] or lot_id}: "
            f"{confidence_label or f'{confidence_score:.2f}'}"
        )
        if pressure_watch_count > 0:
            summary["note"] += f", {pressure_watch_count} trykkfunn"
        return summary

    if component_type == "primer":
        if confidence_score >= 0.65 or data_points >= 3:
            summary["level"] = "ok"
            summary["title"] = "Tennhettelot er godt kjent"
        elif confidence_score > 0.0 or data_points > 0:
            summary["level"] = "info"
            summary["title"] = "Tennhettelot er under læring"
        else:
            summary["level"] = "unknown"
            summary["title"] = "Tennhettelot mangler læring"
        summary["note"] = (
            f"tennhettelot {summary['lot_number'] or lot_id}: "
            f"{confidence_label or f'{confidence_score:.2f}'}"
        )
    return summary
...
def build_profile_evidence_summary(
    ...
    summary: dict[str, Any] = {
        ...
        "evidence_label": "Ingen evidens ennå",
        "trend_label": "Ingen trenddata ennå",
        "history_level": "unknown",
        "history_title": "Historikk mangler",
        "lot_label": "",
        "lot_level": "unknown",
        "lot_title": "Lotter ikke verifisert",
        "lot_notes": "",
        "lot_actions": [],
        "brass_level": "unknown",
        "brass_title": "",
    ...
    if pressure_level == "critical":
        ...
        summary["analysis_title"] = "Aktiv analyse viser hoy trykkrisiko"
    elif internal_level == "critical":
        summary["analysis_level"] = "warning"
        summary["analysis_title"] = "Aktiv analyse viser hoy internballistisk risiko"
    elif pressure_level == "warning":
        summary["analysis_level"] = "info"
        summary["analysis_title"] = "Aktiv analyse viser smal trykkmargin"
    elif internal_level == "warning":
        summary["analysis_level"] = "info"
        summary["analysis_title"] = (
            "Aktiv analyse viser krevende fyllingsgrad eller forbrenning"
        )
    ...
    try:
        ...
        grt_row = dict(grt_rows[0]) if grt_rows else None
    except Exception:
        grt_row = None
    ...
    summary["has_reference_data"] = bool(grt_row)
    ...
    if grt_row:
        summary["predicted_velocity_fps"] = _safe_float(
            grt_row.get("predicted_velocity")
        )
    ...
    if isinstance(component_context, dict):
        ...
        if bullet_context.get("selected_lot_number") or bullet_context.get(
            ...
            lot_bits.append(
                f"kulelot {bullet_context.get('selected_lot_number') or bullet_context.get('lot_number')}"
            )
        if powder_context.get("selected_lot_number") or powder_context.get(
            "lot_number"
        ):
            lot_bits.append(
                f"kruttlot {powder_context.get('selected_lot_number') or powder_context.get('lot_number')}"
            )
        if primer_context.get("selected_lot_number") or primer_context.get(
            "lot_number"
        ):
            lot_bits.append(
                f"tennhettelot {primer_context.get('selected_lot_number') or primer_context.get('lot_number')}"
            )
        if brass_context.get("lot_number"):
            lot_bits.append(f"hylselot {brass_context.get('lot_number')}")
    ...
    for item in lot_summaries:
        ...
        if component_type == "bullet":
            if "mangler måleserie" in title:
                lot_actions.append(
                    "Kulelot: mål gjerne 10-20 kuler for snitt, SD og lengde."
                )
            elif level == "info":
                lot_actions.append(
                    "Kulelot: utvid måleserien før du tolker små seating- og nodeforskjeller hardt."
                )
        elif component_type == "powder":
            if level == "warning":
                lot_actions.append(
                    "Kruttlot: kjør en kort verifiseringsserie for fart og trykk før videre finjustering."
                )
            elif level == "info":
                lot_actions.append(
                    "Kruttlot: bygg mer læring med chrono før du stoler på små charge-forskjeller."
                )
        elif component_type == "primer":
            if level in {"unknown", "info"}:
                lot_actions.append(
                    "Tennhettelot: bekreft ES/SD og trykkrespons med en kort kontrollserie."
                )
        if note and "0 målinger" in note and component_type == "bullet":
            lot_actions.append("Kulelot: ingen måleserie registrert ennå.")
    ...
    if any(level == "warning" for level in lot_levels):
        ...
        summary["lot_title"] = "Lotter krever oppfølging"
    elif sum(1 for level in lot_levels if level == "ok") >= 2:
        summary["lot_level"] = "ok"
        summary["lot_title"] = "God lot-kunnskap"
    elif any(level in {"ok", "info"} for level in lot_levels):
        summary["lot_level"] = "info"
        summary["lot_title"] = "Noe lot-kunnskap"

    evidence_bits = []
    if summary["chrono_count"]:
        evidence_bits.append(f"{summary['chrono_count']} chronoøkter")
    if summary["accuracy_count"]:
        evidence_bits.append(f"{summary['accuracy_count']} presisjonstester")
    if summary["pressure_count"]:
        evidence_bits.append(f"{summary['pressure_count']} trykkhendelser")
    else:
        evidence_bits.append("ingen trykkhendelser registrert")
    if summary["has_reference_data"]:
        evidence_bits.append("predikert referansedata finnes")
    if summary["best_group_mm"] is not None:
        evidence_bits.append(
            f"beste gruppe {format_group_size_mm(summary['best_group_mm'])}"
        )
    if summary["avg_group_mm"] is not None:
        evidence_bits.append(
            f"snittgruppe {format_group_size_mm(summary['avg_group_mm'])}"
        )
    ...
    if summary["latest_velocity_fps"] is not None:
        trend_bits.append(
            f"siste chrono {format_velocity_fps(summary['latest_velocity_fps'])}"
        )
    ...
    if summary["latest_group_mm"] is not None:
        trend_bits.append(
            f"siste gruppe {format_group_size_mm(summary['latest_group_mm'])}"
        )
    if summary["predicted_velocity_fps"] is not None:
        trend_bits.append(
            f"predikert {format_velocity_fps(summary['predicted_velocity_fps'])}"
        )
    ...
    summary["evidence_label"] = (
        ", ".join(evidence_bits) if evidence_bits else "ingen data ennå"
    )
    summary["trend_label"] = (
        " | ".join(trend_bits) if trend_bits else "ingen trenddata ennå"
    )
    ...
    if summary["pressure_count"]:
        ...
        summary["history_title"] = "Historikk viser risiko"
    elif summary["analysis_level"] == "warning":
        summary["history_level"] = "warning"
        summary["history_title"] = str(
            summary["analysis_title"] or "Aktiv analyse viser risiko"
        )
    elif summary["lot_level"] == "warning":
        summary["history_level"] = "warning"
        summary["history_title"] = "Historikk viser lot-risiko"
    elif (
...
def build_load_card_summary(db, ammo_profile_id: int | None) -> dict[str, Any]:
    summary: dict[str, Any] = {
        "title": "Ladningskort",
        "subtitle": "Velg en ladning for å se sammendrag.",
        "component_line": "",
        "measurement_line": "",
        "status_line": "",
        "projectile_line": "",
    ...
    if not profile:
        summary["subtitle"] = "Kunne ikke lese ladningsprofil."
        return summary
    ...
    if case and case.get("name"):
        component_bits.append(f"Hylse {case.get('name')}")
    if isinstance(component_context, dict):
        bullet_context = (
            component_context.get("bullet")
            if isinstance(component_context.get("bullet"), dict)
    ...
    grt_row = dict(grt_rows[0]) if grt_rows else {}

    predicted_velocity = _safe_float(grt_row.get("predicted_velocity"))
    ...
    if predicted_velocity is not None:
        measurement_bits.append(f"predikert {format_velocity_fps(predicted_velocity)}")
    if measured_velocity is not None:
        measurement_bits.append(f"målt {format_velocity_fps(measured_velocity)}")
    if evidence.get("best_group_mm") is not None:
        measurement_bits.append(
            f"beste gruppe {format_group_size_mm(evidence['best_group_mm'])}"
        )
    ...
    if evidence.get("chrono_count"):
        detail_lines.append(f"{int(evidence['chrono_count'])} chronoøkter")
    if evidence.get("accuracy_count"):
        detail_lines.append(f"{int(evidence['accuracy_count'])} presisjonstester")
    if evidence.get("pressure_count"):
        detail_lines.append(f"{int(evidence['pressure_count'])} trykkhendelser")
    if (
    ...
    summary.update(
        {
            "title": str(profile.get("name") or "Ladningskort"),
...
def _normalize_source_label(source: str, context: dict[str, Any]) -> str:
    ...
    if source == "gordon_dump_import":
        return "Referanseimport" if not mismatch else "Referanseimport med avvik"
...
def _build_status(
    ...
    if mismatch:
        return "Avvik i kildefil"
    if predicted_velocity is not None and measured_velocity is not None:
        return "Verifisert hos oss"
    if predicted_velocity is not None and chrono_count:
        return "Har referanse + chrono"
    if predicted_velocity is not None:
        return "Har referansedata"
    if chrono_count:
        return "Har chrono-data"
    return "Profil"
...
def _build_status_from_evidence(
    *,
    ...
    if pressure_count:
        return "Historikk viser risiko"
    if lot_level == "warning":
        return "Lotter krever oppfølging"
    if (
    ...
    if lot_level == "ok" and (chrono_count or accuracy_count):
        return "Verifisert med lot-kunnskap"
    if predicted_velocity is not None and chrono_count:
        return "Har referanse + chrono"
    if lot_level in {"ok", "info"} and not (chrono_count or accuracy_count):
        return str(evidence_summary.get("lot_title") or "Har lot-kunnskap")
...
def build_owned_gordon_rows(db) -> list[dict[str, Any]]:
    ...
    for profile in profiles or []:
        ...
        has_gordon = profile.get("predicted_velocity") is not None or source.startswith(
            "gordon_"
        )
        if not has_gordon:
            continue
