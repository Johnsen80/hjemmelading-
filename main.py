...
from src.logging_config import configure_logging, get_logger, get_log_dir
...
def main():
        ...
        try:
            ...
            try:
                ...
                if app_inst is not None:
                    try:
                        QMessageBox.critical(None, 'Uventet feil', 'Et uventet problem oppstod. Se debug_err.log for detaljer.')
                    except Exception:
                        pass
            except Exception:
                pass
    ...
    try:
        ...
        try:
            ...
            fonts_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'HjemmeladingApp', 'resources', 'fonts')
        loaded = []
            ...
            try:
                ...
                for ttf_path in glob.glob(os.path.join(fonts_dir, '*.ttf')):
                    try:
                        ...
                        try:
                            append_exception(f'Failed to register font with matplotlib: {ttf_path}', e)
                        except Exception:
                            pass
    ...
    def still_waiting():
        ...
        if not splash.isVisible():
            splash_label.setText("FEIL: Splash-vinduet er usynlig! Sjekk skjerminnstillinger og driver.")
            splash.show()
            app.processEvents()
    QTimer.singleShot(10000, still_waiting)
    QTimer.singleShot(30000, lambda: splash_label.setText("Startup failed or is blocked.\nNo window appeared.\nCheck debug_err.log and debug_app.log."))
